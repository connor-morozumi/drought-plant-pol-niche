...
```{r}
dat = visitation %>% select(site, specimen_number, gross_ID, plant, year)

# create a "matching" dataframe to generate drought / non-drought column
# based on year (2018 and 2020 were drought years)
yearmatch = data.frame(year = c(2016, 2017, 2018, 2019, 2020),
            drought = c("non-drought", "non-drought", "drought", 
                        "non-drought", "drought"))

# left_join to add drought column to `dat`
dat = left_join(dat, yearmatch, by = "year")
rm(yearmatch) # cleanup
...
## WEB.SMALL    
  ...
  # calculate metrics
sm.results= calc.metrics(sm.temp, netlevel.indices)  
...
# DOWNSAMPLING LOOP  
  ...
  for(i in 1:n.rep){
    ...
    results[i,] = calc.metrics(temp, netlevel.indices)
  }
    # all done!
...
constrained.nondrought.results= calc.metrics(edge2net(non.drought.inx.comm), netlevel.indices)  

calc.metrics(edge2net(drought.inx.comm), netlevel.indices)  
```
- drought becomes a net that is 37 plants by 62 pols
- non drought becomes a net that is 36 plants by 62 pols
  * the difference is Boechera stricta because it is in both years but get eliminated in non drought due to not finding the pollinator it interacts with in drought
...
for(k in 1:nsamps){ # 100
        ...
        for(l in 1:comboz){
            ...
            tempnet = edge2net(select(comb, gross_ID, plant))
...
```{r}
years= (yr.deg + yr.con) / (yr.nodf + yr.h2)
...
flowers %>% 
 # group_by(year) %>% 
  ggplot(., aes(x= year, y= total_flowers))+ 
  geom_boxplot()+
